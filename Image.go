...
func AcquireImageFromFile(fs http.FileSystem, path string) (img *Image, e error) {
	...
	if r, ok = imageRegistry[k]; !ok {
		...
		if n != size {
			return nil, NewError(fmt.Sprintf("Read %d bytes from file, expected %d", n, size))
		}
		img := platformNewImageFromBytes(buffer)
		if img == nil {
			return nil, NewError(fmt.Sprintf("Unable to load image from %s", path))
		}
		img.key = k
		img.id = nextImageID
		nextImageID++
...
func AcquireImageFromURL(url string) (img *Image, e error) {
	...
	if r, ok = imageRegistry[url]; !ok {
		...
		if img == nil {
			return nil, NewError(fmt.Sprintf("Unable to load image from %s", url))
		}
		img.key = url
		img.id = nextImageID
		nextImageID++
...
func AcquireImageFromData(data *ImageData) (img *Image, e error) {
	...
	if img == nil {
		return nil, NewError("Unable to load image from data")
	}
	imageRegistryLock.Lock()
	defer imageRegistryLock.Unlock()
	img.id = nextImageID
...
func (img *Image) AcquireImageBounds(bounds Rect) (image *Image, e error) {
	...
	if image == nil {
		return nil, NewError("Unable to create image")
	}
	imageRegistryLock.Lock()
	defer imageRegistryLock.Unlock()
