...
func (p *Precision) HorizontalSpacing() float64 {
	return p.hSpacing
}

// SetHorizontalSpacing sets the horizontal spacing between columns.
func (p *Precision) SetHorizontalSpacing(spacing float64) *Precision {
	p.hSpacing = spacing
	return p
}

// VerticalSpacing returns the vertical spacing between rows.
func (p *Precision) VerticalSpacing() float64 {
	return p.vSpacing
}

// SetVerticalSpacing sets the vertical spacing between rows.
func (p *Precision) SetVerticalSpacing(spacing float64) *Precision {
	p.vSpacing = spacing
	return p
}
...
func (p *Precision) layout(location geom.Point, hint geom.Size, move, useMinimumSize bool) geom.Size {
	...
	if p.columns > 0 {
		...
		if len(children) > 0 {
			...
			totalSize.Width += p.hSpacing * float64(p.columns-1)
			totalSize.Height += p.vSpacing * float64(p.rows-1)
...
func (p *Precision) adjustColumnWidths(width float64, grid [][]Widget) []float64 {
	availableWidth := width - p.hSpacing*float64(p.columns-1)
	expandCount := 0
	widths := make([]float64, p.columns)
	minWidths := make([]float64, p.columns)
		...
		for i := 0; i < p.rows; i++ {
			...
			if data != nil {
				...
				if hSpan > 1 {
					...
					w := data.cacheSize.Width - spanWidth - float64(hSpan-1)*p.hSpacing
					if w > 0 {
						if p.equal {
					...
					if !data.hGrab || minimumWidth != 0 {
						...
						w -= spanMinWidth + float64(hSpan-1)*p.hSpacing
						if w > 0 {
	...
	if p.equal {
		...
		if width != NoHint && expandCount != 0 {
			columnWidth = math.Max(minColumnWidth, math.Floor(availableWidth/float64(p.columns)))
		}
		for i := 0; i < p.columns; i++ {
						...
						if data != nil {
							...
							if hSpan > 1 {
								...
								if !data.hGrab || minimumWidth != 0 {
									...
									w -= spanWidth + float64(hSpan-1)*p.hSpacing
									if w > 0 {
										p.apportionExtra(w, j, spanExpandCount, hSpan, expandColumn, widths)
									}
								}
...
func (p *Precision) apportionExtra(extra float64, base, count, span int, expand []bool, values []float64) {
	if count == 0 {
		values[base] += extra
	} else {
...
func (p *Precision) wrap(width float64, grid [][]Widget, widths []float64, useMinimumSize bool) {
	if width != NoHint {
		for j := 0; j < p.columns; j++ {
			for i := 0; i < p.rows; i++ {
				data := p.getData(grid, i, j, false)
				if data != nil {
					if data.sizeHint.Height == NoHint {
						...
						currentWidth += float64(hSpan-1) * p.hSpacing
						if currentWidth != data.cacheSize.Width && data.hAlign == draw.AlignFill || data.cacheSize.Width > currentWidth {
							data.computeCacheSize(grid[i][j], geom.Size{Width: math.Max(data.minCacheSize.Width, currentWidth), Height: NoHint}, useMinimumSize)
...
func (p *Precision) adjustRowHeights(height float64, grid [][]Widget) []float64 {
	availableHeight := height - p.vSpacing*float64(p.rows-1)
	expandCount := 0
	heights := make([]float64, p.rows)
	minHeights := make([]float64, p.rows)
		...
		for j := 0; j < p.columns; j++ {
			...
			if data != nil {
				...
				if vSpan > 1 {
					...
					h := data.cacheSize.Height - spanHeight - float64(vSpan-1)*p.vSpacing
					if h > 0 {
					...
					if !data.vGrab || minimumHeight != 0 {
						...
						h -= spanMinHeight + float64(vSpan-1)*p.vSpacing
						if h > 0 {
					...
					if data != nil {
						...
						if vSpan > 1 {
							...
							if !data.vGrab || minimumHeight != 0 {
								...
								h -= spanHeight + float64(vSpan-1)*p.vSpacing
								if h > 0 {
...
func (p *Precision) positionChildren(location geom.Point, grid [][]Widget, widths []float64, heights []float64) {
	gridY := location.Y
	for i := 0; i < p.rows; i++ {
		gridX := location.X
		for j := 0; j < p.columns; j++ {
			...
			if data != nil {
				...
				cellWidth += p.hSpacing * float64(hSpan-1)
				childX := gridX
				...
				cellHeight += p.vSpacing * float64(vSpan-1)
				childY := gridY
