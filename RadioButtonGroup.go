...
// RadioButtonGroup is used to ensure only one RadioButton in a group is selected at a time.
type RadioButtonGroup struct {
	members []*RadioButton
}

// NewRadioButtonGroup creates a new group for the specified buttons. Each button is removed from
// any other group it may be in and placed in the newly created one.
func NewRadioButtonGroup(buttons ...*RadioButton) *RadioButtonGroup {
	group := &RadioButtonGroup{members: buttons}
...
func (g *RadioButtonGroup) Add(button *RadioButton) {
	if button.group != nil {
...
func (g *RadioButtonGroup) Remove(button *RadioButton) {
	if button.group == g {
...
func (g *RadioButtonGroup) Select(button *RadioButton) {
	if button.group == g {
