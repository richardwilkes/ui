...
func (pm *PopupMenu) Init() {
	...
	pm.Sizes = func(hint geom.Size) (min, pref, max geom.Size) {
	...
	pm.OnPaint = func(gc graphics.Context, dirty geom.Rect, inLiveResize bool) {
		...
		gc.DrawAttributedTextConstrained(bounds, pm.attributedString(), graphics.TextFill)
	}
}
...
func (pm *PopupMenu) AddSeparator() {
	pm.items = append(pm.items, &sepMarker{})
}
...
func (pm *PopupMenu) attributedString() *graphics.AttributedString {
	title := ""
	if pm.selectedIndex >= 0 && pm.selectedIndex < len(pm.items) {
		title = fmt.Sprintf("%v", pm.items[pm.selectedIndex])
	}
	return graphics.NewAttributedString(title, pm.TextColor(), pm.Theme.Font)
}

func (pm *PopupMenu) measureConstrained(hint geom.Size) geom.Size {
	var largest geom.Size
	for _, one := range pm.items {
		size, _ := graphics.NewAttributedString(fmt.Sprintf("%v", one), color.Black, pm.Theme.Font).MeasureConstrained(hint)
