...
func (a *AttributedString) toPlatform() C.CFMutableAttributedStringRef {
	...
	for _, ra := range a.attributes {
		...
		switch ra.attr.key {
		case fontAttribute:
			C.CFAttributedStringSetAttribute(as, r, C.kCTFontAttributeName, C.CFTypeRef(ra.attr.value.(*font.Font).PlatformPointer()))
		...
		case alignmentAttribute:
			align := C.CTTextAlignment(ra.attr.value.(uint8))
...
func (a *AttributedString) platformMeasure(size geom.Size) (actual geom.Size, fit int) {
