...
// Context provides a graphics context for drawing into.
type Context interface {
	...
	DrawImage(img *image.Image, where geom.Point)
	// DragImage in the bounds, scaling if necessary.
	DrawImageInRect(img *image.Image, bounds geom.Rect)
	...
	DrawAttributedText(x, y float32, str *AttributedString, mode TextMode) geom.Size
	...
	DrawAttributedTextConstrained(bounds geom.Rect, str *AttributedString, mode TextMode) (actual geom.Size, fit int)
