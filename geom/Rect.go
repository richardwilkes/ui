...
func (r *Rect) Intersect(other Rect) {
	if r.IsEmpty() || other.IsEmpty() {
		...
		x := maths.MaxFloat32(r.X, other.X)
		y := maths.MaxFloat32(r.Y, other.Y)
		w := maths.MinFloat32(r.X+r.Width, other.X+other.Width) - x
		h := maths.MinFloat32(r.Y+r.Height, other.Y+other.Height) - y
		if w > 0 && h > 0 {
			r.X = x
			r.Y = y
			r.Width = w
...
func (r *Rect) Union(other Rect) {
	...
	if e1 && e2 {
		...
		*r = other
		...
		x := maths.MinFloat32(r.X, other.X)
		y := maths.MinFloat32(r.Y, other.Y)
		r.Width = maths.MaxFloat32(r.X+r.Width, other.X+other.Width) - x
		r.Height = maths.MaxFloat32(r.Y+r.Height, other.Y+other.Height) - y
		r.X = x
		r.Y = y
	}
}
