...
// Monochrome creates a new opaque Color from a grayscale value in the range 0-255.
func Monochrome(level int) Color {
	return MonochromeA(level, 1)
}

// MonochromeA creates a new Color from a grayscale value in the range 0-255 and an alpha value
// in the range 0-1.
func MonochromeA(level int, alpha float32) Color {
...
func RGBA(red, green, blue int, alpha float32) Color {
	return Color(scaleAndClamp(alpha))<<24 | Color(clamp0To255(red)<<16|clamp0To255(green)<<8|clamp0To255(blue))
}
...
func (c Color) SetRedIntensity(red float32) Color {
	return RGBA(scaleAndClamp(red), c.Green(), c.Blue(), c.AlphaIntensity())
}
...
func (c Color) SetGreenIntensity(green float32) Color {
	return RGBA(c.Red(), scaleAndClamp(green), c.Blue(), c.AlphaIntensity())
}
...
func (c Color) SetBlueIntensity(blue float32) Color {
	return RGBA(c.Red(), c.Green(), scaleAndClamp(blue), c.AlphaIntensity())
}
